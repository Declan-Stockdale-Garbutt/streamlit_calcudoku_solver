...
if __name__ == '__main__':
    ...
    if add_selectbox == 'Calcudoku solver':
        ...
        with header:
            st.title('Calcudoku/KenKen puzzle solver application')

        #select image
        with image_selection:
            st.write("")
            st.header("Select puzzle and folder location")
            st.write("Puzzles can be found in github repo or screenshots from either the app or https://api.razzlepuzzles.com/calcudoku")
            st.write("Some other kenken puzzles may work but optical character recognition may fail")
            st.write("")
            st.write("")
            st.subheader("Please upload an image or select one from example puzzle folder")
        ...
        with folder_selection:
            st.write(' ')
            st.write('Choose a folder to store images')

            clicked = st.button('Choose a folder')

            # Necessary to save folder and image states and not reload
            # init dirname state
            if "dirname" not in st.session_state:
                st.session_state.dirname = False

            # init session state
            if "clicked_state" not in st.session_state:
                st.session_state.clicked_state = False

            if clicked:
                st.session_state.clicked_state = True
                dirname = st.text_input('Selected folder:', filedialog.askdirectory(master=root))
                st.session_state.dirname = dirname

            if clicked == False and st.session_state.dirname:
                dirname = st.text_input('Selected folder:', st.session_state.dirname,key = 'loop')


        # Diplay image and see if rotation is necessary
        with image_display:

            if image_upload is not None and st.session_state.dirname is not False:
                ...
                filepath = st.session_state.dirname+'/calcudoku_solver_img.jpg'
                            ...
                            with save_container:
                                ...
                                if st.session_state.state_is_solved:
                                    ...
                                    if colour:
                                        ...
                                        solved_image_picture = img_proc.write_solved_puzzle(to_print,filepath,number_of_cells,colour)
                                        st.image(solved_image_picture)

                                    # save puzzle output will be written over every time program is run
                                    st.write('Solved puzzle has been saved to ',filepath)
                                    solved_puzzle_save = cv2.imwrite(filepath, solved_image_picture)
    ...
    if add_selectbox == 'Requirements':
        ...
        st.write('For calcudoku solver to work properly, a version of Tesseract must be installed ')
        st.write('Tesseract can be downloaded from the link below')
        ...
        st.write('While both do similar things, the different font and the requirement for perfect character recognition required two seperate models.The process beind training the models can be found under the "More detail on training tesseract model" option.')
    ...
    if add_selectbox == "How it works":
        ...
        st.write("Puzzles can be screenshots from either the website https://api.razzlepuzzles.com/calcudoku or the app available on both the Apple store and the Google play store")
        st.write("The image is then kept in the local machines ram as a bytes type object. This is jus the was streamlit handles input images at this point.")

        st.subheader("Opencv2 Computer vision")
        st.write("The python computer vision library Opencv2 is used to process and detect various features within the uploaded image. For opencv2 to function correctly, the image as a bytes type object must be saved locally and read into opencv2, this is the reason for having a folder selection.")

        st.write("Once the folder has been defined, the image is saved and read by opencv2.")
        st.write("")

        st.subheader("Finding the playing grid")
        st.write("The image is converted into back and white")
        ...
        st.write("The next step is find the coordinates of the largest area enclosed within white lines")
        st.write('Assuming there is minimal noise, this should be the playing grid')
        ...
        st.write(" The grid is then rotated and scaled to make it a square")

        st.subheader("Finding number of cells")
        st.write("The number of cells is found firstly by rastering down the grid vertically and recording the distance between white pixels (border of cells) to a list. The same is performed  with a horizontal raster")
        st.write("Small values are removed from the list  which may occur due to encountered noise, a math operator or a number in a given cell")
        st.write("The minimum value of the remaining results is recorded as the cell length in pixels, other values should be multiple of this value e.g. two cells, three cells ")
        st.write(" The number of cells is found by dividing the shape of the size of the grid by the size of the box in pixels and rounded to the nearest integer")
        ...
        st.write(" We will iterate over pixels across potential cell boundaries to determine if they are connected")
        st.write(" If no border is encountered (e.g. a white pixel) along the entire search length, then the cells must be connected")
        st.write(" Instead of starting at the start of the cell, to save some computation, we'll start at the 3/4 distance and search until we hit the first 1/4 of the next box")
        st.write(" By doing this we are only searching half the box distance and should avoid hitting math operators which occur in the top left of the cell")

        st.write("This process is first performed vertically then horizontally")
        st.write("Graph networking is implemented to find cells such as L shaped or squares where there's a box connected vertically and horizontally creating two lists of connected cells")
        st.write(" The returned result is a list of connected cells where each box coordinates is a tuple")
        st.write("All the math operators are in the top left most square of all connected cells so sorting the returned list by these allows us to get cells containing math operators")
        ...
        st.write('There is minor tweaking on the predicions for the math_cells with multiple math operators occuring in some predicitons such as 7+x  or +7+ where we only want 7+ etc')
        st.write(" The final output from both OCR results is a list of the connected cells operator number (e.g. [(0,0),(0,1)] add 7)")
        ...
        st.write('The output from the kenken solver is an numpy array and the list of centre pixel coordinates is a flat list')
        ...
        st.write('A few modificaitons must be made first, largely as opencv2 starts the text at the bottom left pixel which is where we have defined the centre, the centre coordinate needs to adjusted by the size of the height of the written text to diplay nicely')
    ...
    if add_selectbox == 'More detail on training tesseract model':
        ...
        st.write('A very similar process was performed on cells containing given values or what I occassionally refer to as solid cells.')
    ...
    if add_selectbox == "Future work":
        ...
        st.write('Goal 3.')
        st.write('Deploy application as a webapp either to streamlit cloud or heroku')
